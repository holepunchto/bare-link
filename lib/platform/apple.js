...
module.exports = async function apple(base, pkg, name, version, opts = {}) {
  ...
  for (const [os, hosts] of archs) {
    ...
    const prebuilds = hosts.map((host) => path.join(base, 'prebuilds', host))

    frameworks.push(await framework(os, base, pkg, name, version, prebuilds))
  }
  ...
  try {
    if (frameworks.length === 1) {
      ...
      const target = path.join(out, path.basename(framework))

      await fs.cp(framework, target)

      return [target]
    } else {
      return [await xcframework(name, version, frameworks, opts)]
    }
  } finally {
...
async function framework(os, base, pkg, name, version, prebuilds) {
  ...
  if (extra.size > 0) {
    ...
    for (const [name, inputs] of extra) {
      await run('lipo', ['-create', '-output', path.join(frameworks, name), ...inputs])
    }
  }
...
<dict>
  ...
  <string>${name.replace(invalidBundleIdentifierCharacter, '-')}.${cleanedVersion}</string>
