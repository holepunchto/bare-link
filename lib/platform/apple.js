...
module.exports = async function apple (base, pkg, name, version, opts = {}) {
  ...
  const archs = new Map([['macos', []], ['ios', []], ['ios-simulator', []]])
  ...
  for (const [, target] of archs) {
    ...
    for (const host of target) prebuilds.push(path.join(base, 'prebuilds', host, `${name}.bare`))

    frameworks.push(await framework(base, pkg, name, version, prebuilds, opts))
  }
...
async function framework (base, pkg, name, version, inputs, opts = {}) {
  ...
  await run('lipo', ['-create', '-output', path.join(framework, `${name}.${version}`), ...inputs], opts)

  const replacements = ['-id', `@rpath/${name}.${version}.framework/${name}.${version}`]

  for await (const { name, version } of dependencies(base, pkg)) {
    const major = version.substring(0, version.indexOf('.'))

    replacements.push(
      '-change', `${name}@${major}.bare`, `@rpath/${name}.${version}.framework/${name}.${version}`
    )
  }

  await run('install_name_tool', [...replacements, path.join(framework, `${name}.${version}`)], opts)

  await fs.writeFile(path.join(framework, 'Info.plist'), plist(name, version))
